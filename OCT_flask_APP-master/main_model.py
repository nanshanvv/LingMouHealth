...
class HFF_block(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        ...
        self.self_attn = nn.MultiheadAttention(embed_dim=ch_int, num_heads=8, dropout=drop_rate)
        self.self_attn_norm = LayerNorm(ch_int, eps=1e-6, data_format="channels_first")
    ...
    def forward(self, l, g, f):
        ...
        # Self-Attention Mechanism
        X_f_attn, _ = self.self_attn(X_f.permute(2, 3, 0, 1).flatten(2), X_f.permute(2, 3, 0, 1).flatten(2), X_f.permute(2, 3, 0, 1).flatten(2))
        X_f_attn = X_f_attn.view(X_f.size(2), X_f.size(3), X_f.size(0), X_f.size(1)).permute(2, 3, 0, 1)
        X_f = X_f + self.self_attn_norm(X_f_attn)
